...
export default function UserData() {
  ...
  useEffect(() => {
    axios({
      ...
      url: `${baseUrl}/api/v1/admin/get-user-data/`,
  ...
  const getCategoriesListAndSetParentCategoryName = (data) => {
    axios({
      ...
      .then((res) => {
        ...
        data.forEach((item) => {
          const obj = categoryList.find((i) => item.currentPackage === i._id);

          console.log('obj', obj);
          // item.parentCategoryName = obj?.name;
          item.currentPackage = obj?.packageName;
  ...
  const handleDeleteContent = () => {
    axios({
      method: 'DELETE',
      url: `${baseUrl}/api/v1/admin/delete-user-data/${currentContentId}`,
      withCredentials: true,
    })
      .then((res) => {
        console.log(res.data);
  ...
  const handleClickName = (item) => {
    navigate(`/dashboard/update-user-data/${item._id}`, { state: item });
  };
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <UserMoreMenu item={row} page="user-data" deleteHandler={handleConfirmationDialogOpen} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
